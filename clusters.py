...
class Clusters:
    ...
    def clusterisePixels(self):
        matrix = self.matrix
        pixels = np.sum(matrix)
        print(pixels)
    ...
    def determineOrder(self):
        ...
        opt = AntOpt(points)
        opt.explore()
        self.order = opt.optimalSol

    def getImportantPoints(self):
        for c in self.clusters:
            c.pixels.sort(key = lambda x: x[0])
            c.keyPoints.append(c.pixels[0])
            c.keyPoints.append(c.pixels[-1])
